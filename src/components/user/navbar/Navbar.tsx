  ...
  return (
    <>
      <div className="h-16 md:h-20 lg:h-24 bg-white flex justify-around items-center px-4 md:px-6 lg:px-8 border-b border-gray-300 relative z-[500]">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <div className="flex gap-3 md:gap-4 text-gray-600">
            ...
            <BiUserCircle
              className="text-xl md:text-2xl cursor-pointer hover:text-black transition"
              onClick={() => {
                setAuthMode("signup"); // open signup first
      ...
      {showAuthModal &&
        (authMode === "signup" ? (
          <Register
            setShowSignup={setShowAuthModal}
            setAuthMode={setAuthMode}
          />
        ) : (
          <Login setShowSignup={setShowAuthModal} setAuthMode={setAuthMode} />
        ))}
    </>
  );
};
